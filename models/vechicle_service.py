...
class VechicleService(models.Model):
    ...
    mobile_number=fields.Char(string="mobile_number",required=True)
    advisor_id=fields.Many2one('res.users',string="advisor",required=True)
    vechicle_no=fields.Char(string="vechicle no",required=True,)
    state=fields.Selection([('draft','draft'),('inprogress','in-progress'),('ready','ready for delivery'),('cancelled','cancelled')],string="state",required=True,tracking=True,default="draft")
    vechicle_image=fields.Image(string="vechicle image",required=True,max_width=1920,max_height=1920)
    ...
    end_date=fields.Date(string="excepted repair date")
